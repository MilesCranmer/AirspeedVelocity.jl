...
function run_benchmarks(
    ...
    return _run_benchmarks(package_spec; output_dir, benchmark_script, tune, exeflags)
end
function run_benchmarks(
    ...
    for spec in package_specs
        run_benchmarks(spec; output_dir, benchmark_script, tune, exeflags)
    end
end

function run_benchmarks(package_name::String, revs::Vector{String}; kws...)
    return _run_benchmarks(
        [PackageSpec(; name=package_name, rev=rev) for rev in revs]; kws...
    )
end
function run_benchmarks(package_name::String, rev::String; kws...)
    return run_benchmarks(package_name, [rev]; kws...)
end
